...
#include <stdlib.h>
#include <string.h>
// Gabriel - altera��o 13/09/2008 - in�cio 
#include <sstream>
using namespace std;
// Gabriel - altera��o 13/09/2008 - fim
...
void PMConstructiveMD::run (PMSolution *s) {
	...
	// AQUI DEVE SER REALIZADA A MINERA��O
	if (construcaoInicial == 0)
	{
		...
		buffer << "./bin/fpmax_hnmp "  // minerador de padroes frequentes
			   ...
			   << arq_padroes.str().c_str(); // arquivo onde ser�o armazenados os padroes
		mine.resume();
	...
	{
	// Caso hajam menos de 16 padroes, fa�a um uso circular dos mesmos
		ti_padroes_iter++;
...
void PMConstructiveMD::sample (PMSolution *s, double fraction, bool stats) {
	...
	//---------------------------------
	s->reset(); //start with an empty solution
	// Gabriel - altera��o 22/07/2008	- in�cio 
	// AQUI DEVE SER !APLICADA! A MINERA��O!!!!
	...
	//s->add(fac, true);
	// Gabriel - altera��o 22/07/2008	- fim
	int count = 0;
	while (s->getP() < p) {
		count++;
...
// Gabriel - altera��o 07/09/2008 - in�cio - contando tempo de construcao
//	fprintf (stdout, "sampletime %.3f\n", t.getTime());
